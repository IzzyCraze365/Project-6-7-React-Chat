...
const RoomIndex = (props) => {
  ...
  function getRoomInfo(roomValue) {
    ...
    let chatroomNameFilter =
      roomArray.filter((room) => roomValue.target.value === room._id)[0]
        ?.name ?? "No Room Selected";
    setRoomName(chatroomNameFilter);
    let chatroomDescriptionFilter =
      roomArray.filter((room) => roomValue.target.value === room._id)[0]
        ?.description ??
